...
def conv_forward(x, w, b, conv_param):
    ...
    # reshaping w as a matrix of rows
    w_reshaped = w.reshape(F, C*HH*WW)
    x_inter = np.zeros((C*HH*WW, H_*W_))
    
    # looping through all images in input
    for i in range(N):
      T = 0
      #loop through all filters and apply them to current image
      for j in range(0, H_p-HH+1, stride):
        for k in range(0, W_p-WW+1, stride):
          x_inter[:,T] = x_padded[i, :, j:j+HH, k:k+WW].reshape(C*HH*WW)
          T+=1
      
      out[i] = (w_reshaped.dot(x_inter) + b.reshape(F,1)).reshape(F, H_, W_)
...
def max_pool_forward(x, pool_param):
    ...
    for i in range(N):
      T = 0
      out_inter = np.zeros((C, H_ * W_))
      for j in range(0, H-HH+1, stride):
        for k in range(0, W-WW+1, stride):
          pooling_region = x[i, :, j:j+HH, k:k+WW].reshape(C,HH*WW)
          out_inter[:,T] = pooling_region.max(axis=1)

          T+=1

      out[i] = out_inter.reshape((C,H_,W_)) 
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
